...
from workspace_manager import list_workspaces, get_workspace, update_workspace
from file_manager import read_file, write_file, delete_file, list_files as fm_list_files
...
try:
    ...
    def ui_context(workspaceId: str) -> dict:
        ...
        return {
            ...
            "viewport": {
                "x": settings.get('viewportX', 0),
                "y": settings.get('viewportY', 0),
    ...
    class SimpleMCP:
                    ...
                    if method == 'tools/list':
                        self._send(rid, {
                            'tools': [
                                ...
                                {'name': 'workspace_read', 'description': 'Read workspace', 'inputSchema': {'type': 'object', 'properties': {'workspaceId': {'type': 'string'}}}},
                                {'name': 'workspace_update', 'description': 'Update workspace', 'inputSchema': {'type': 'object', 'properties': {'workspaceId': {'type': 'string'}, 'windows': {'type': 'array'}, 'settings': {'type': 'object'}}}},
                                {'name': 'file_read', 'description': 'Read file', 'inputSchema': {'type': 'object', 'properties': {'workspaceId': {'type': 'string'}, 'path': {'type': 'string'}}}},
                                {'name': 'file_write', 'description': 'Write file', 'inputSchema': {'type': 'object', 'properties': {'workspaceId': {'type': 'string'}, 'path': {'type': 'string'}, 'content': {'type': 'string'}}}},
                                {'name': 'file_list', 'description': 'List directory', 'inputSchema': {'type': 'object', 'properties': {'workspaceId': {'type': 'string'}, 'dir': {'type': 'string'}}}},
                                {'name': 'file_delete', 'description': 'Delete file', 'inputSchema': {'type': 'object', 'properties': {'workspaceId': {'type': 'string'}, 'path': {'type': 'string'}}}},
                                {'name': 'ui_context', 'description': 'Get UI context', 'inputSchema': {'type': 'object', 'properties': {'workspaceId': {'type': 'string'}}}},
                            ]
                        })
        ...
        def _call_tool(self, name, args):
            if name == 'workspace_list':
                ...
                if 'windows' in args: ws['windows'] = args['windows']
                if 'settings' in args: ws['settings'] = args['settings']
                update_workspace(args['workspaceId'], ws)
                return {'updatedAt': ws.get('updatedAt', '')}
            elif name == 'file_read':
                return read_file(args['workspaceId'], args['path'])
            elif name == 'file_write':
                return write_file(args['workspaceId'], args['path'], args['content'])
            elif name == 'file_list':
                return fm_list_files(args['workspaceId'], args.get('dir', ''))
